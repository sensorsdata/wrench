...
test('test getUA function', (t) => {
  ...
  var val = getUA();
  t.deepEqual(val, {}, 'call getUA() when navigator.userAgent == "" and then it returns {}');
  ...
  testCases.forEach((testCase) => {
    ...
    t.deepEqual(
      val,
      testCase.expect,
      `when current browser is ${testCase.browser} call getUA() and it returns ${JSON.stringify(testCase.expect)}`
    );
  });
