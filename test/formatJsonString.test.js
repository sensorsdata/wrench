...
test('test map encodeDates', (t) => {
  ...
  t.ok(spy.calledOnce, 'when formatJsonString get called, then JSON.stringify will be called once.');
  t.equal(val, target, `when call formatJsonString(${JSON.stringify(obj)}), then it returns '{\n"a": 1\n}'`);
  ...
  t.equal(val, '{"a":1}', 'when call formatJsonString("{ a: 1 }"), then it returns "{"a": 1}"');
